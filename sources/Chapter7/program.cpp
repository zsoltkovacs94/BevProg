...
constexpr char powch = 'p'; // hatványozás tokenjének kind-ja
constexpr char sqrtch = 's'; // gyökvonás tokenjének kind-ja
constexpr char delcch = '#'; // változó deklarálás átírva 
const string declkey = "let"; //constexpr string since C++20 only
const string powkw = "pow"; // a hatványozáshoz beirandó kulcsszó
const string sqrtkw = "sqrt"; // a gyökvonáshoz beirandó kulcsszó
const string quitkw = "exit"; // kilépés kulcsszava
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			if (is_declared(s))
					return Token(number, get_value(s));
    			if (s == sqrtkw) return Token{sqrtch};
    			if (s == powkw) return Token{powch};
...
try {
	...
	define_name("k", 1000);	// k definiálása 
...
double squareroot(){
	...
	t = ts.get();
	if(t.kind != ')') error(") expected in sqrt");
	if(a<0) error("negative number in sqrt()");
	return sqrt(a);
}
